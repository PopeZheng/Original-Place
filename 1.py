...
if selection=='1':
    ...
    if number=='1':
        ...
        corstring1= base64.b64encode(string1.encode('utf-8'))
        print("您所得的加密串为:",str(corstring1,'utf-8'))
    if number=='2':
        string2=input('请您输入需要解密的字符串\n')
        try:
            corstring2=base64.b64decode(string2.encode('utf-8'))
            print('您所获得的解密串为:',str(corstring2,'utf-8'))
        except:
            print('您的输入非法，请输入合法的字符串！\n')
if selection=='2':
    try:
        string3=input('''请您输入需要处理的字符串
 注意输入格式为{"a":1}型的格式\n''')
